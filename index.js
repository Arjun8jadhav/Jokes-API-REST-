...
app.get("/jokes/:id",(req,res)=>{
  ...
  const founjoke=jokes.find((joke)=>joke.id===num);
  res.json(founjoke);

})
app.get("/filter",(req,res)=>{
    const type=req.query.type;
    const array1=jokes.filter((joke)=>joke.jokeType==type);
    res.json(array1);
})
...
app.put("/jokes/:id",(req,res)=>{
     ...
     const type1=req.body.typel;
     const user={
      id: id1,
      jokeText: text1,
      jokeType: type1,
     }
     const index=jokes.findIndex((joke)=> joke.id===id1);
     jokes[index]=user;
     res.json(user);
})
app.patch("/jokes/:id",(req,res)=>{
  const id1=parseInt(req.params.id);
  const existing=jokes.find((joke)=> joke.id===id1);
  const new1={
    id: id1,
    jokeText: req.body.text || existing.jokeText,
    jokeType: req.body.type || existing.jokeType,
  }
  const index=jokes.findIndex((joke)=> id1===joke.id);
  jokes[index]=new1;
  res.json(new1);
...
app.delete("/all",(req,res)=>{
       ...
       if(userKey===masterKey){
        ...
        res.status(401).json({error : `your not authorized person`});
       }
       res.json("hello");
})
